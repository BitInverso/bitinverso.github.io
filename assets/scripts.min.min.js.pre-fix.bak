...
window.addEventListener('pointercancel', () => { try { resetAllCards(); } catch(e){} });
window.addEventListener('blur', () => { try { resetAllCards(); } catch(e){} });
, 750);
}
cards.forEach(card => {
if (supportsHover) {
card.addEventListener('mouseenter', () => { setActiveCard(card); });
card.addEventListener('mouseleave', () => { if (activeCard===card) { activeCard.classList.remove('is-flipped'); activeCard=null; } });
